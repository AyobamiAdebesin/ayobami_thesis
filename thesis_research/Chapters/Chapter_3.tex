...
Next, we consider the effect of ill-conditioning on stability for a non-symmetric decomposition like the $LU$ decomposition. The $LU$ decomposition of $A - \sigma B$ is performed using the \texttt{linalg.lu\_factor} function in SciPy, which employs partial pivoting. With a shift $\sigma = 1.5 \times 10^3$ not close to a generalized eigenvalue as before, we obtain small residuals for eigenvalues close to or smaller than the shift in magnitude in the case of the well-conditioned problem as shown in Fig~\ref{fig:LU-Well-Conditioned}. These small residuals are close to machine precision. The theoretical bound (red curve) is a plot of $10^{-12}\lvert 1-\lambda_i/\sigma\rvert$, indicating that the residuals scales with $\lvert 1-\lambda_i/\sigma\rvert$. Again, this factor occurs in the error bounds described in \ref{thrm:ResidualBounds}. 
