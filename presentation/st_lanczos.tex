...
\begin{frame}
	...
	\begin{itemize}
		\item $QZ$ Algorithm, (C. B. Moler and G. W. Stewart, 1972) - Useful for non-symmetric dense problems. So it is not suitable for sparse symmetric problems since it does not preserve symmetry. It is also very slow.
		
		\item Simultaneous Diagonalization of $A$ and $B$, (S. Chandrasekaran 2000) - Uses simulatenous diagonalization of $A$ and $B$ using congruence to compute eigenvalues. It is slow and expensive.
	\end{itemize}
\end{frame}
...
\begin{frame}[allowframebreaks]{Outline}
	...
	\begin{itemize}
		...
		\item For a symmetric matrix $A \in \mathbb{R}^{m\times m}$ and an initial vector $\mathbf{v}_1$, the algorithm constructs the $n$-dimnensional Krylov subspace
		\begin{equation*}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item \textit{To what extent do these bounds carry over to the Spectral Transform Lanczos algorithm?}
...
\begin{frame}
  \frametitle{Setting up a Generalized Eigenvalue Problem}
	...
	Given a diagonal matrix $D \in \mathbb{R}^{m \times m}$ with predefined eigenvalues, and a regularization hyperparameter $\delta$, the following algorithm sets up a generalized eigenvalue problem
	\begin{algorithmic}[1]
...
\begin{frame}[allowframebreaks]{Outline}
  ...
  \item Generate matrices $A$ and $B$ with $\texttt{GENERATE\_MATRIX}$ function so that 
   ...
   \item We use a tolerance of $10^{-9}$ for the Ritz pair convergence for all experiments.
  \end{itemize}
\end{frame}
...
\begin{frame}[allowframebreaks]{Outline}
  \frametitle{$LU$ Decomposition (Moderate Shift with ill-conditioning)}
  
  \begin{figure}
  	\caption{Residual plot with moderate shift $\sigma=1.5 \times 10^3$ and  $\kappa_2(A- \sigma B) = 5.5\times 10^{12}$}
  	\vspace{1ex}
  	\begin{minipage}{0.45\textwidth} % Adjust the width to control spacing
  		\centering
  		\includegraphics[scale=.25]{/home/ayobami/ayobami_thesis/code/plots/LU/gen_residual_mod_shift.png}
  		\subcaption{}
  	\end{minipage}%
  	\hfill
  	\begin{minipage}{0.45\textwidth}
  		\centering
  		\includegraphics[scale=.25]{/home/ayobami/ayobami_thesis/code/plots/LU/ritz_residual_mod_shift.png}
  		\subcaption{}
  	\end{minipage}
  	...
  	\subcaption{}
  \end{figure}
  The computation gave a Lanczos decomposition residual of $1.30 \times 10^{-7}$. Plot ($a$) shows the generalized relative residual. The curve is approximation of the plot of $10^{-12}|1-\lambda_i/\sigma|$. Plot ($b$) is the relative Ritz residuals. Plot ($c$) is the best achievable residual for an idealized eigenvector.
\end{frame}

\begin{frame}[allowframebreaks]{Outline}
	\frametitle{$LU$ Decomposition (Large Shift with ill-conditioning)}
	
	 \begin{figure}
		\caption{Residual plot with large shift $\sigma=1.5 \times 10^6$ and $\kappa_2(A- \sigma B) = 3.10\times 10^{8}$}
		\vspace{1ex}
		\begin{minipage}{0.45\textwidth} % Adjust the width to control spacing
			\centering
			\includegraphics[scale=.25]{/home/ayobami/ayobami_thesis/code/plots/LU/gen_residual_large_shift.png}
			\subcaption{}
		\end{minipage}%
		\hfill
		\begin{minipage}{0.45\textwidth}
			\centering
			\includegraphics[scale=.25]{/home/ayobami/ayobami_thesis/code/plots/LU/ritz_residual_large_shift.png}
			\subcaption{}
		\end{minipage}
		...
		\centering % 
		\includegraphics[scale=.25]{/home/ayobami/ayobami_thesis/presentation/Plots/LU/residual_lu_bl.png}
		\subcaption{}
	\end{figure}
	For a large shift, we obtain a Lanczos decomposition residual of as $6.14 \times 10^{-10}$. Plot ($a$) shows the generalized relative residual with the curve approximately given by
	$10^{-14}|(1-\lambda_i/\sigma)(1-\sigma/\lambda_i)|$. Plot ($b$) is the relative Ritz residuals. Plot ($c$) shows that eigenvalues that several orders of magnitude smaller than the shift cannot achieve a small residuals, except for those close to the shift.
\end{frame}
...
\begin{figure}
	...
	\begin{minipage}{0.45\textwidth} % Adjust the width to control spacing
		\centering
		\includegraphics[scale=.25]{/home/ayobami/ayobami_thesis/code/plots/WDW/gen_residual_mod_shift.png}
		\subcaption{}
	\end{minipage}%
	\hfill
	\begin{minipage}{0.45\textwidth}
		\centering
		\includegraphics[scale=.25]{/home/ayobami/ayobami_thesis/code/plots/WDW/ritz_residual_mod_shift.png}
		\subcaption{}
	\end{minipage}
	...
	\centering
	\includegraphics[scale=.25]{/home/ayobami/ayobami_thesis/presentation/Plots/eigdecomp/residual_eig_bs.png}
	\subcaption{}
\end{figure}
...
\begin{figure}
	...
	\begin{minipage}{0.45\textwidth} % Adjust the width to control spacing
		\centering
		\includegraphics[scale=.25]{/home/ayobami/ayobami_thesis/code/plots/WDW/gen_residual_large_shift.png}
		\subcaption{}
	\end{minipage}%
	\hfill
	\begin{minipage}{0.45\textwidth}
		\centering
		\includegraphics[scale=.25]{/home/ayobami/ayobami_thesis/code/plots/WDW/ritz_residual_large_shift.png}
		\subcaption{}
	\end{minipage}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Does conditioning affects the residual bounds?
		\item  Set $\delta = 10^{-12}$ to increase the conditioning of $A-\sigma B$ and compare generalized residuals for a moderate shift $\sigma = 1.5\times 10^3$ with those obtained when $\delta = 10^{-3}$
	\end{itemize}
\end{frame}
...
\begin{frame}[allowframebreaks]
	...
	\begin{figure}
		\caption{Residual plot comparison with a moderate shift $\sigma = 1.5\times 10^3$}
		\vspace{1ex}
		\begin{minipage}{0.45\textwidth} % Adjust the width to control spacing
			\centering
			\includegraphics[scale=.25]{/home/ayobami/ayobami_thesis/code/plots/WDW/gen_residual_ill_cond.png}
			\subcaption{}
		\end{minipage}%
		\hfill
		\begin{minipage}{0.45\textwidth}
			\centering
			\includegraphics[scale=.25]{/home/ayobami/ayobami_thesis/code/plots/WDW/gen_residual_well_cond.png}
			\subcaption{}
		\end{minipage}
	\end{figure}
	\pagebreak
...
\begin{frame}[allowframebreaks]{Outline}
	...
	\begin{minipage}{0.45\textwidth} % Adjust the width to control spacing
		\centering
		\includegraphics[scale=.25]{/home/ayobami/ayobami_thesis/code/plots/LU/gen_residual_ill-cond.png}
		\subcaption{}
	\end{minipage}%
	\hfill
	\begin{minipage}{0.45\textwidth}
