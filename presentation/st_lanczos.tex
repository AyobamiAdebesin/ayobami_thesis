...
\begin{frame}
	...
	\begin{itemize}
        \item $QZ$ Algorithm, (C. B. Moler and G. W. Stewart, 1972) - Useful for non-symmetric dense problems. So it is not suitable for sparse symmetric problems since it does not preserve symmetry. It is \added{stable but} also \rep{relatively}{very} slow.
		
        \item Simultaneous Diagonalization of $A$ and $B$, (S. Chandrasekaran 2000) - Uses \rep{simultaneous}{simulatenous} diagonalization of $A$ and $B$ using congruence to compute eigenvalues. It is slow and \rep{of uncertain stability}{expensive.}
	\end{itemize}
\end{frame}
  ...
  \frametitle {Interesting Questions}
	\begin{center}
		\begin{itemize}
			...
			\item \large \rep{How does symmetry in the decomposition of $A-\sigma B$ impact the spectral transformation Lanczos algorithm?}{Does the spectral transformed problem respects symmetry in the decomposition of $A - \sigma B$?}
		\end{itemize}
	\end{center}
\end{frame}
...
\begin{frame}[allowframebreaks]{Outline}
  ...
  \begin{figure}
  	...
  	\begin{minipage}{0.45\textwidth}
  		\centering
  		\includegraphics[scale=.25]{../code/plots/LU/ritz_residual_mod_shift.png}
  		\subcaption{}
  	\end{minipage}
...
\begin{frame}[allowframebreaks]{Outline}
	\frametitle{$LU$ Decomposition (Large Shift with ill-conditioning)}
	 ...
	 \begin{figure}
		...
		\begin{minipage}{0.45\textwidth} % Adjust the width to control spacing
			\centering
			\includegraphics[scale=.25]{../code/plots/LU/gen_residual_large_shift.png}
			\subcaption{}
		\end{minipage}%
		\hfill
		\begin{minipage}{0.45\textwidth}
			\centering
			\includegraphics[scale=.25]{../code/plots/LU/ritz_residual_large_shift.png}
			\subcaption{}
		\end{minipage}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Does conditioning \rep{affect}{affects} the residual bounds?
...
\begin{frame}
  ...
  {\bf Cons:}
  \begin{itemize}
  \item The \added{generalized} eigenvector computation is not unconditionally stable.
  \end{itemize}
\end{frame}
