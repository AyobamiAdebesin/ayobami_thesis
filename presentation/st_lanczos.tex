...
\begin{frame}[allowframebreaks]{Outline}
	...
	\begin{algorithmic}[1]
		\Function{\textsc{Spectral\_Lanczos}}{$A, B, n, \sigma, tol$}
		\State Choose an arbitrary vector $\mathbf{b}$; set $\mathbf{q}_1 = \mathbf{b}/ \|\mathbf{b}\|_2$
		\State Initialize $Q_n = [\mathbf{q}_1]$, $\mathbf{q}_0 = \mathbf{0}$, $\delta_0 = 0$
		\State Initialize $\gamma = [\,], \delta = [\,]$ \Comment{Arrays for $T_n$ entries}
		\State $LU = (A - \sigma B)$ \Comment{Precompute the $LU$ factorization}
		\State Factor: $B = CC^T$ \Comment{Cholesky factorization}
		\For{$j = 1, 2, \dots, n$}
		\State $\mathbf{u} = C\mathbf{q}_j$ \Comment{Matrix vector product}
		\State $\mathbf{v} = \textsc{LinearSolve}(LU, \mathbf{u})$ \Comment{Forward/Backward substitution}
		\State $\mathbf{v} = C^T \mathbf{v}$ \Comment{Matrix-vector product}
		\State $\gamma_j = \mathbf{q}_j^T \mathbf{v} $ \Comment{Diagonal entry $\gamma_j$}
		\State $\mathbf{v} = \mathbf{v} - \delta_{j-1}\mathbf{q}_{j-1} - \gamma_j \mathbf{q}_j$ \Comment{$3$-term recurrence}
		\State $\mathbf{v} = \mathbf{v} - \sum_{i=1}^j (\mathbf{q}_i^T \mathbf{v})\mathbf{q}_i$ \Comment{Full reorthogonalization}
		\State $\delta_{j} = \|\mathbf{v}\|_2$ \Comment{Off-diagonal entry $\delta_{j}$}
		\If{$\delta_{j} < tol $}
		\State \textbf{break}
		\EndIf
		\State $\mathbf{q}_{j+1} := \mathbf{v} / \delta_{j}$
		\State Append $\mathbf{q}_{j+1}$ to $Q_n$ and $\gamma_j$, $\delta_{j}$ to $\gamma$, $\delta$
		\EndFor
		\State Truncate $Q_n$ to first $n$ vectors: $Q_n = [\mathbf{q}_1, \dots, \mathbf{q}_n]$
		\State \textbf{return} $Q_n$, $\gamma$, $\delta$, $\mathbf{q}_{n+1}$
		\EndFunction
	\end{algorithmic}
\end{frame}
...
\begin{frame}[allowframebreaks]{Outline}
	...
	\begin{figure}
		...
		\centering
		\includegraphics[scale=.25]{../code/plots/WDW/best_residual_mod_shift.png}
		\subcaption{}
	\end{figure}
...
\begin{frame}[allowframebreaks]{Outline}
	...
	\begin{figure}
		...
		\centering
		\includegraphics[scale=.25]{../code/plots/WDW/best_residual_large_shift.png}
		\subcaption{}
	\end{figure}
